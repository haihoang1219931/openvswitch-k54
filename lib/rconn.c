...
void handle_bandwidth(struct ofp_vendor_bandwidth * vendor_bandwitdh){
    printf("vendor_bandwitdh %04x %d bytes\r\n",htons(vendor_bandwitdh->header.length),htons(vendor_bandwitdh->header.length));
...
void handle_port_modify(struct ofp_vendor_port_modify * port_modify){
    printf("vendor_port_modify %04x %d bytes\r\n",htons(port_modify->header.length),htons(port_modify->header.length));
    printf("type: %02x\r\n",port_modify->type);
    printf("port_id: %02x\r\n",port_modify->port_id);
...
void handle_vendor(unsigned char *buffer){
    ...
    //getOFP_TYPE(oh->type);
    //printf("htons(oh->length): %04x %d\r\n",htons(oh->length),htons(oh->length));
    //dump((const unsigned char *)buffer,0,htons(oh->length));
